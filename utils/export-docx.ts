...
function normalizePartner(p: any): Partner {
  ...
  // STRICT CHECK: Prioritize explicit flags. Avoid fuzzy matching on contact roles (e.g. "Coordinator of Studies").
  const isCoord =
    src.isCoordinator === true ||
    src.is_coordinator === true ||
    (src.role && String(src.role).toLowerCase().includes('coordinator')) || // Allow "Project Coordinator"
    (src.role && String(src.role).toLowerCase() === 'applicant');     // Exact match only for role
...
export async function generateDocx(proposal: FullProposal): Promise<{ blob: Blob; fileName: string }> {
  try {
    ...
    if (logoUrl) {
      // Logic to handle both data URI and regular URL for logo
      try {
        if (logoUrl.startsWith('data:image')) {
          ...
          // Placeholder for future remote image fetching if needed
          // For now we skip remote URLs if not fetched, but the logic 
          // in fetchImageAsBase64 is available for use.
        }
      } catch (e) {
    ...
    docChildren.push(createSectionHeader("Table of Contents", 1));
    docChildren.push(new TableOfContents("Summary", {
      hyperlink: true,
    ...
    finalDocument.forEach((section: DisplaySection) => {
      ...
      if (isPartners && p.partners?.length > 0) {
        ...
        sorted.forEach((pt) => {
          ...
          const label = partner.isCoordinator ? "Applicant (Coordinator)" : "Partner";
                ...
                rows: [
                  new TableRow({
                    children: [
                      new TableCell({
                        children: [new Paragraph({
                          children: [new TextRun({ text: "CONFIDENTIAL Proposal", size: 16, color: "666666", font: FONT })],
                          alignment: AlignmentType.LEFT
                        })],
                      ...
                      new TableCell({
                        children: [new Paragraph({
                          children: [
                            new TextRun({ text: "Page ", size: 16, color: "666666", font: FONT }),
                            new TextRun({ children: [PageNumber.CURRENT], size: 16, color: "666666", font: FONT }),
                            new TextRun({ text: " of ", size: 16, color: "666666", font: FONT }),
        ...
        paragraphStyles: [
          {
            id: "Normal",
            name: "Normal",
            run: { font: FONT, size: BODY_SIZE, color: COLOR_SECONDARY },
          ...
          new TableCell({
            children: [
              new Paragraph({
                children: [
                  new TextRun({
                    text: i === 0 ? "Coordinator" : "Partner", // STRICT: Only 1st is Coordinator
                    bold: i === 0,
                    font: FONT,
                    size: 18
                  })
