...
export function MobilityProposalViewer({ proposalId, onBack }: ProposalViewerPageProps) {
    const {
        proposal,
        setProposal,
        loading,
    ...
    // Auto-fix logicMode if we are in this viewer
    useEffect(() => {
        if (proposal && proposal.logic_mode !== 'mobility') {
            const updateMode = async () => {
                const updated = { ...proposal, logic_mode: 'mobility' };
                try {
                    await saveProposal(updated);
                    console.log("Proposal logic_mode persisted as 'mobility'");
                } catch (e) {
                    console.error("Failed to persist logic_mode", e);
                }
            };
            updateMode();
        }
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center h-screen gap-4 bg-background">
                <p className="text-muted-foreground animate-pulse font-medium italic">Hydrating Mobility Intelligence...</p>
            </div>
        );
    }
    ...
    const logicMode = 'mobility';
    const sections = assembleDocument(proposal, logicMode);
