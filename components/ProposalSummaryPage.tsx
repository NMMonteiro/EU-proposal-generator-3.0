        ...
        const loadProposal = async () => {
            ...
            try {
                ...
                if (data.funding_scheme_id) {
                    try {
                        const resScheme = await fetch(`${serverUrl}/funding-schemes/${data.funding_scheme_id}`, {
                            headers: {
                                'Authorization': `Bearer ${publicAnonKey}`
                            }
                        });
                        if (resScheme.ok) {
                            const scheme = await resScheme.json();
                            data.fundingScheme = scheme;
                            data.funding_scheme = scheme;
                            // Firestore scheme has sections directly on it
                            data.layout = scheme;
                            data.layout_id = scheme.id;
                        }
                    } catch (e) {
