...
export function IdeasStep({ analysisResult, sourceUrl, userPrompt, onSelectIdea, onBack }: IdeasStepProps) {
  ...
  const isMobility = analysisResult.logic_mode === 'mobility';

  return (
    <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in duration-700">
      {/* Logic Mode & Knowledge Context */}
      <div className="flex flex-col md:flex-row gap-4">
        <Card className={`flex-1 ${isMobility ? 'bg-amber-50 border-amber-200' : 'bg-indigo-50 border-indigo-200'} shadow-lg shadow-slate-100 overflow-hidden relative`}>
          <div className={`absolute top-0 left-0 w-1 h-full ${isMobility ? 'bg-amber-600' : 'bg-indigo-600'}`}></div>
          <CardContent className="p-5 flex items-center gap-5">
            <div className={`${isMobility ? 'bg-amber-600/10 text-amber-700' : 'bg-indigo-600/10 text-indigo-700'} p-3 rounded-2xl`}>
              {isMobility ? <Calculator className="h-6 w-6" /> : <Rocket className="h-6 w-6" />}
            </div>
            <div className="flex-1">
              <p className={`text-sm font-bold tracking-tight ${isMobility ? 'text-amber-900' : 'text-indigo-900'}`}>
                System Mode: {isMobility ? 'Mobility Flows Expert' : 'Standard Project Architect'}
              </p>
              <p className={`text-xs mt-1 ${isMobility ? 'text-amber-700/70' : 'text-indigo-700/70'}`}>
                {isMobility
                  ? 'Optimized for Erasmus+ KA122/KA121/KA131 structures with activity-based budgeting.'
                  : 'Optimized for Work Package-based structures (Horizon, KA210/220, etc).'}
              </p>
            </div>
            <Badge className={`${isMobility ? 'bg-amber-600' : 'bg-indigo-600'} text-white border-none px-3 py-1`}>
              {isMobility ? 'MOBILITY' : 'STANDARD'}
            </Badge>
          </CardContent>
        </Card>
