...
export function StandardProposalViewer({ proposalId, onBack }: ProposalViewerPageProps) {
    const {
        proposal,
        setProposal,
        loading,
    ...
    useEffect(() => {
        if (proposal && proposal.logic_mode !== 'standard') {
            const updateMode = async () => {
                const updated = { ...proposal, logic_mode: 'standard' };
                try {
                    await saveProposal(updated);
                    console.log("Proposal logic_mode persisted as 'standard'");
                } catch (e) {
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center h-screen gap-4 bg-background">
                ...
                <p className="text-muted-foreground animate-pulse font-medium">Hydrating Project Intelligence...</p>
            </div>
        );
    }
