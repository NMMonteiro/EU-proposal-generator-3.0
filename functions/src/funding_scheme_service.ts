...
export const enrichFundingScheme = async (schemeId: string) => {
    const doc = await db.collection('funding_schemes').doc(schemeId).get();
    if (!doc.exists) throw new Error('Scheme not found');
    ...
    await db.collection('funding_schemes').doc(schemeId).update({
...
export const syncSchemeFromContent = async (content: string, type: 'template' | 'example') => {
    ...
    if (data.matchedSchemeId && data.confidence > 0.7) {
        ...
        if (Object.keys(updates).length > 0) {
            await db.collection('funding_schemes').doc(data.matchedSchemeId).update(updates);
