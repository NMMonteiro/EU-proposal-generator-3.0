...
exports.handleAiEdit = handleAiEdit;
 ...
 */
async function handleAiEdit(params) {
    const { proposalId, sectionKey, sectionTitle, currentContent, instruction } = params;
    const model = (0, ai_service_1.getGeminiModel)({ temperature: 0.7 });
    ...
    const data = (0, utils_1.extractJSON)(text);
    if (proposalId && data.newContent) {
        const proposal = await (0, proposal_service_1.loadFullProposal)(proposalId);
        if (proposal) {
            ...
            proposal.dynamic_sections = dynSections;
 ...
 */
async function handleCopilotChat(params) {
    const { proposalId, message, history = [] } = params;
    const proposal = await (0, proposal_service_1.loadFullProposal)(proposalId);
    if (!proposal)
        throw new Error("Proposal not found");
    const { KnowledgeRetriever } = await Promise.resolve().then(() => __importStar(require('./knowledge_retriever')));
    ...
    - Partners: ${JSON.stringify(proposal.partners?.map((p) => p.name))}
    ...
    const chatHistory = history.map((ms) => ({
    ...
    const data = (0, utils_1.extractJSON)(responseText);
    if (!data)
        return { response: "I encountered an error parsing the AI response.", actions: [] };
        ...
        if (actions.length > 0) {
            ...
            for (const action of actions) {
                ...
                if (action.type === 'update_section') {
                    ...
                    if (updates.summary)
                        proposal.summary = updates.summary;
                    if (updates.settings) {
                    ...
                    if (updates.mobilityMetadata) {
                        ...
                        proposal.mobility_metadata = { ...(proposal.mobility_metadata || {}), ...updates.mobilityMetadata };
                    }
                }
                    ...
                    if (logicMode === 'mobility' && mobilityRules) {
                        ...
                        const osIdx = budget.findIndex((b) => (b.category || b.item || '').toLowerCase().includes('organizational support'));
                        const newItem = {
...
//# sourceMappingURL=ai_editor.js.map