...
exports.extractNumericBudget = void 0;
exports.buildPhase2Prompt = buildPhase2Prompt;
exports.buildRelevancePrompt = buildRelevancePrompt;
exports.buildProposalPrompt = buildProposalPrompt;
const extractNumericBudget = (text) => {
    ...
    if (highIntentMatch) {
        ...
        return parseRobustNumber(val || highIntentMatch[1]);
    }
...
exports.extractNumericBudget = extractNumericBudget;
...
function buildPhase2Prompt(summary, constraints, userPrompt, fundingScheme, examples = []) {
...
function buildRelevancePrompt(url, urlContent, constraints, ideas, userPrompt) {
    return `Evaluate relevance of ideas against: ${urlContent.substring(0, 2000)}. Return JSON {score, justification}`;
}
function buildProposalPrompt(idea, summary, constraints, partners = [], userPrompt, fundingScheme, logicModeOverride, examples = []) {
    ...
    // Budget detection
    let budgetNum = (0, exports.extractNumericBudget)(userPrompt || '');
    if (!budgetNum) {
        budgetNum = (0, exports.extractNumericBudget)(constraints.budget || '') || (0, exports.extractNumericBudget)(constraints.budgetLimit || '');
    }
    ...
    const partnerDictionary = partners.map((p, i) => `[PARTNER ${i + 1}]: "${p.name}" (${p.country || 'N/A'})`).join('\n');
    const directivePlaybook = Array.isArray(expertRules)
        ? expertRules.map((r) => `- [${r.rule || r.topic || 'Directive'}]: ${r.guidance || r.description}`).join('\n')
    ...
    const sectionInstructions = allSections.map((s) => {
        ...
        let pillar = "";
        if (s.label.toLowerCase().includes('relevance') || s.label.toLowerCase().includes('context'))
            pillar = "RELEVANCE";
        else if (s.label.toLowerCase().includes('impact') || s.label.toLowerCase().includes('dissemination'))
            pillar = "IMPACT & SUSTAINABILITY";
        else if (s.label.toLowerCase().includes('design') || s.label.toLowerCase().includes('implementation') || s.label.toLowerCase().includes('work plan'))
            pillar = "QUALITY OF DESIGN";
        else if (s.label.toLowerCase().includes('partnership') || s.label.toLowerCase().includes('team') || s.label.toLowerCase().includes('cooperation'))
            pillar = "QUALITY OF PARTNERSHIP";
        if (pillar) {
      ...
      3. Define Coordination Mechanisms: "Project Steering Committee (PSC)" for decision making; "Digital Workspaces" (Asana/Teams) for daily work.`;
        ...
        const relevantRule = Array.isArray(expertRules) && expertRules.find((r) => s.label.toLowerCase().includes(r.topic?.toLowerCase()) ||
