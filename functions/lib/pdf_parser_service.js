...
exports.importExamplePdf = exports.importLibraryPdf = exports.importPartnerPdf = void 0;
const ai_service_1 = require("./ai_service");
const utils_1 = require("./utils");
const importPartnerPdf = async (fileBuffer, fileName, fileSize) => {
    ...
    const model = (0, ai_service_1.getGeminiModel)({ temperature: 0.1, maxOutputTokens: 4096 });
...
Return ONLY the JSON object, no additional text or markdown formatting.`;
    ...
    const extractedData = (0, utils_1.extractJSON)(result.response.text());
    ...
    if (extractedData.relevantProjects && typeof extractedData.relevantProjects === 'object') {
        if (Array.isArray(extractedData.relevantProjects)) {
            extractedData.relevantProjects = extractedData.relevantProjects.map((p) => {
    ...
    if (extractedData.experience && typeof extractedData.experience === 'object') {
        if (Array.isArray(extractedData.experience)) {
            extractedData.experience = extractedData.experience.map((item) => {
    ...
    if (extractedData.staffSkills && typeof extractedData.staffSkills === 'object') {
        if (Array.isArray(extractedData.staffSkills)) {
            extractedData.staffSkills = extractedData.staffSkills.map((item) => {
                if (typeof item === 'object' && item !== null) {
                    return Object.entries(item).map(([k, v]) => `**${k}**: ${v}`).join(', ');
                }
                return String(item);
    ...
    if (extractedData.keywords && typeof extractedData.keywords === 'string') {
        extractedData.keywords = extractedData.keywords.split(',').map((k) => k.trim()).filter(Boolean);
    }
    else if (!Array.isArray(extractedData.keywords)) {
        extractedData.keywords = [];
    }
    const { upsertPartner } = await Promise.resolve().then(() => __importStar(require('./partner_service')));
...
exports.importPartnerPdf = importPartnerPdf;
const importLibraryPdf = async (fileBuffer, fileName) => {
    ...
    const extracted = (0, utils_1.extractJSON)(responseText);
    ...
    const { db } = await Promise.resolve().then(() => __importStar(require('./firebase_db')));
    const { embedText } = await Promise.resolve().then(() => __importStar(require('./ai_service')));
...
exports.importLibraryPdf = importLibraryPdf;
const importExamplePdf = async (fileBuffer, fileName) => {
    const base64Data = fileBuffer.toString('base64');
    ...
    const exampleData = (0, utils_1.extractJSON)(result.response.text());
    const { syncSchemeFromContent } = await Promise.resolve().then(() => __importStar(require('./funding_scheme_service')));
...
exports.importExamplePdf = importExamplePdf;
//# sourceMappingURL=pdf_parser_service.js.map