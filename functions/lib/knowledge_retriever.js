...
class KnowledgeRetriever {
     ...
     */
    async getRelevantKnowledge(query, limit = 8) {
        try {
            if (!query)
                return { content: '', sources: [] };
            console.log(`[RAG-Firestore] Semantic search for: "${query.substring(0, 50)}..."`);
            // 1. Generate embedding for query
            const queryEmbedding = await (0, ai_service_1.embedText)(query);
            ...
            const content = filteredResults.map((chunk) => `
### EXPERT DIRECTIVE: ${chunk.sourceName || chunk.source_name} (${chunk.metadata?.type || 'Guideline'}) [Sim: ${(chunk.similarity * 100).toFixed(1)}%]
${chunk.content}
`).join('\n');
            const sources = Array.from(new Set(filteredResults.map((r) => String(r.sourceName || r.source_name))));
            return { content, sources };
        }
        catch (e) {
    ...
    async getFallbackKnowledge(query, limit) {
...
`).join('\n');
     ...
     */
    static extractSmartKeywords(text) {
        if (!text)
            return [];
