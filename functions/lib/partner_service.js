...
exports.deletePartner = exports.upsertPartner = exports.getPartner = exports.listPartners = void 0;
...
const listPartners = async () => {
    const snap = await firebase_db_1.db.collection('partners').orderBy('name', 'asc').get();
    const list = [];
    snap.forEach(doc => {
...
const getPartner = async (id) => {
    if (!id)
        return null;
    const doc = await firebase_db_1.db.collection('partners').doc(id).get();
    if (!doc.exists)
        return null;
    return mapPartner({ ...doc.data(), id: doc.id });
};
exports.getPartner = getPartner;
const upsertPartner = async (body) => {
    let id = body.id;
    if (!id || !(0, utils_1.isUUID)(id)) {
        id = require('crypto').randomUUID();
    }
    const docRef = firebase_db_1.db.collection('partners').doc(id);
...
const deletePartner = async (id) => {
    ...
    await firebase_db_1.db.collection('partners').doc(id).delete();
    return { success: true };
};
exports.deletePartner = deletePartner;
//# sourceMappingURL=partner_service.js.map